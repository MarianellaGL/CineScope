...
export const CinemaMarker = ({ cinema }: CinemaMarkerProps) => {
  const setSelectedCinema = useMapStore((state) => state.setSelectedCinema);
  const selectedCinema = useMapStore((state) => state.selectedCinema);
      ...
      <div className="relative">
        <div
          className={`flex items-center justify-center w-12 h-12 rounded-full transition-all duration-300 ${
            isSelected
              ? "bg-primary scale-125 shadow-2xl shadow-primary/50"
              : "bg-slate-800 hover:bg-primary/80 hover:scale-110 shadow-lg"
          }`}
        >
          <svg
        ...
        <div
          className={`absolute left-1/2 -translate-x-1/2 top-full w-0 h-0 border-l-8 border-r-8 border-t-8 border-l-transparent border-r-transparent ${
            isSelected ? "border-t-primary" : "border-t-slate-800"
          }`}
        ></div>
      </div>
    </AdvancedMarker>
